...
A focused release that brings full reasoning-effort support to the qwen3.8-max model family across every API conversion path the proxy implements. qwen3.8-max accepts a three-level effort domain — `low`, `medium`, `xhigh` — and notably rejects the `high` value that most other reasoning models understand; every path now clamps `high` (and anything above `medium` that is not already `xhigh`) to `xhigh` before the request leaves the proxy. On the native Responses passthrough, the forwarder rewrites `reasoning.effort` in place; on the Codex-to-Anthropic bridge, qwen3.8-max bypasses the Anthropic `thinking` block format entirely and emits `reasoning_effort` plus `enable_thinking` instead; on the Claude-to-Anthropic passthrough, the proxy derives an effort level from the incoming `thinking` or `output_config` fields and injects the same pair; and on the Codex-to-Chat-Completions bridge, a new "qwen" effort-value mode maps the Codex effort ladder onto the model's accepted subset. The model also joins `supports_reasoning_effort()` so the Anthropic-to-Responses and Anthropic-to-Chat conversion paths apply the same clamping.

**Stats**: 2 commits | 10 files changed | +368 insertions | -64 deletions

### Added

- **qwen3.8-max Reasoning Effort Support Across All Proxy Paths**: The qwen3.8-max family (including `qwen3.8-max-preview`) now has its reasoning effort correctly translated on every conversion bridge. The model accepts only `low`, `medium`, and `xhigh` — it rejects `high` — so a dedicated clamping function maps `high`, `xhigh`, and `max` uniformly to `xhigh` while preserving `low` and `medium`. Paths covered: (1) native Responses passthrough — the forwarder clamps `reasoning.effort` in the request body before it reaches the upstream; (2) Codex → Anthropic — qwen3.8-max skips the Anthropic `thinking`/`budget_tokens`/`adaptive` machinery and instead emits `reasoning_effort` + `enable_thinking: true`, with `none`/`off`/`disabled` suppressing the field entirely; (3) Claude → Anthropic passthrough — the proxy resolves an effort from the incoming `thinking.type`/`budget_tokens` or `output_config.effort`, clamps it, and injects `reasoning_effort` + `enable_thinking`; (4) Codex → Chat Completions — a new `"qwen"` effort-value mode in the reasoning config maps `minimal`/`low` → `low`, `medium` → `medium`, everything else → `xhigh`, and the model's Codex reasoning config declares `effort_param: "reasoning_effort"`, `thinking_param: "enable_thinking"`, `output_format: "reasoning_content"`; (5) Anthropic → Responses and Anthropic → Chat — the shared `clamp_reasoning_effort_for_model()` is applied after effort resolution. Non-qwen models are unaffected; the clamping is a no-op passthrough for every other family.

### Upgrade notes

- No database schema migration in this release.
- The qwen3.8-max effort clamping is applied at the proxy layer only; providers that connect directly without local routing are unaffected.
- `high` is deliberately mapped to `xhigh` rather than `medium`, because qwen3.8-max's `xhigh` is the closest semantic equivalent to the "extended reasoning" intent that `high` carries on other models.
